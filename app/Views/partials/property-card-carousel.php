...
$showFeatured = \App\Support\UnitPricing::listingShowsFeatured($property);
...
?>
<div class="shrink-0 w-[272px] snap-start bg-white rounded-2xl overflow-hidden border border-slate-200/90 shadow-[0_14px_44px_-20px_rgba(15,23,42,0.35)] hover:shadow-[0_22px_50px_-22px_rgba(15,23,42,0.32)] hover:-translate-y-0.5 transition flex flex-col group">
  <div class="relative aspect-[4/3] overflow-hidden bg-slate-100">
    ...
    <div class="absolute bottom-3 left-3 z-[7]" x-data="{ item: <?= $compareJson ?> }">
      <button type="button"
        ...
        <span x-text="$store.compare.isSelected(item.property_id, item.unit_id) ? 'เลือกแล้ว' : 'เทียบ'"></span>
      </button>
    </div>
    <?php endif; ?>
  </div>
  <a href="<?= e($pUrl) ?>" class="p-4 flex flex-col flex-1 text-left">
    ...
    <?php endif; ?>
    <div class="flex items-center gap-1 text-[12px] text-slate-500 mt-1.5">
      ...
      ?>
      <span class="truncate"><?= $locMain !== '' ? e($locMain) . ($prov !== '' ? ', ' . e($prov) : '') : e($prov !== '' ? $prov : 'กาญจนบุรี') ?></span>
    </div>
    <?php if ($amenLine !== ''): ?>
      <div class="mt-2 flex flex-wrap gap-x-2 gap-y-1 text-[11px] text-slate-600 font-semibold leading-snug">
        <span class="inline-flex items-center gap-1"><i data-lucide="<?= e($amenIcon) ?>" class="w-3.5 h-3.5 text-slate-400"></i><?= e($amenLine) ?></span>
      </div>
    <?php else: ?>
    ...
    <div class="mt-auto pt-4 flex items-end justify-between gap-2 border-t border-slate-100">
      <div>
        ...
        <?php \App\Core\View::partial('partials/unit-price-card', ['pricingUnit' => $listingUid > 0 ? $property : array_merge($property, ['price' => $cardPrice, 'capacity_max' => (int)($property['_unit_cap_max'] ?? 0), 'capacity_min' => (int)($property['_unit_cap_min'] ?? 0)])]); ?>
      </div>
      <?php $crc = (int)$property['rating_count']; ?>
      <?php if ($crc > 0): ?>
      <div class="flex items-center gap-1 text-[12px] font-bold shrink-0">
        ...
        <span class="text-slate-800"><?= number_format((float)$property['rating_avg'], 1) ?></span>
        <span class="text-slate-400 font-semibold">(<?= $crc ?>)</span>
      </div>
      <?php else: ?>
